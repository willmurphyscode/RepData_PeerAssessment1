...
typicalDay$interval <- as.integer(unlist(strsplit(as.character(typicalDay$intervalDay), ".", fixed=TRUE))[1])
typicalDay$weekday <- as.integer(unlist(strsplit(as.character(typicalDay$intervalDay), ".", fixed=TRUE)[2]))

imputeValues <- function(df) {
   intervalDayInput <- paste(as.character(df$interval), as.character(df$dayOfWeeek), sep =  ".")
   df$intervalDay = intervalDayInput
   df <- df %>% left_join(typicalDay, c("intervalDay" = "intervalDay"))
   df <- rename(df, steps = steps.x, interval = interval.x, imputedValue = steps.y)
